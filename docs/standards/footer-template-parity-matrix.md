...
**This file is a _snapshot_, not live status — re-verify before relying on it.** The
historical rows ("Completed First-Wave Matrix") are settled; the forward-looking rows
("Next Planned Wave", draft PR #343) drift over time and must be re-checked.

| Field                | Value                                                                   |
| -------------------- | ----------------------------------------------------------------------- |
| Status               | 🟡 Snapshot — verify before relying                                     |
| Last verified        | 2026-06-16 (against footer-only PR #62 / `40e0175…`)                    |
| Imported to ffcadmin | 2026-06-30 (content unchanged; external refs not re-verified at import) |
| Re-verify by         | 2026-07-16 — treat as **stale** after this date                         |
| Source of truth      | the linked issues/PRs in the owning repos below — **not this file**     |

**How to refresh (the only way to make this current):**

1. Open every external link below; confirm each PR/issue state and that the pinned commit
   SHAs still point at the owning repo's `main`.
2. Correct any moved row — edit only the cells you actually verified.
3. Set **Last verified** to today's date and **Re-verify by** to 30 days out.
4. Log the refresh in the coordination issue (#442).
...
Coordination issue: #442. Content last verified 2026-06-16 (originally draft PR
#443); imported into ffcadmin here — see the **Freshness** block at the top for
verification status and the refresh procedure. Agents should open draft PRs and
must not merge.
