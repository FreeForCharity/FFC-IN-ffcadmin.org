...
| Routines (research preview)   | Cloud sessions triggered by cron, an API trigger endpoint, or GitHub PR/Release events (rich filters). Included in Max — **15 runs/day**; subscription login required                                | Not yet. **Caveat:** routines run as the creating user's identity, and a green run status means the infrastructure ran, not that the task succeeded |
