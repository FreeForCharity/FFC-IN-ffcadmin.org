...
| 5   | Orchestration  | No `claude-code-action` / event-driven agent runs; every session is manually initiated                               | 🟡       | M      | Phase 2   |
...
| 7   | Governance     | No dedicated bot identity for unattended runs; automation would run as a volunteer's personal account                | 🟠       | M      | Phase 2   |
...
| 12  | Observability  | No cost/turn budgets on agent runs; spend is bounded only by human attention                                         | 🟡       | S      | Phase 3   |
